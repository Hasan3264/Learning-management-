<div class="px-4">
    <h1 class="text-center text-2xl p-4 ">{{$quiz->name}}</h1>
    @php
        $i=1
    @endphp
           ...
           <div class="flex gap-4">
               @forEach($answerOpitons as $option)
                   <div class="flex items-center pl-4  rounded">
                       ...
                       <label for="answer-{{$option}}-{{$question->id}}" class="w-full py-4 cursor-pointer ml-2 text-sm font-medium">{{$question->$option}}</label>
                   </div>
               @endforeach
           </div>
       </div>
